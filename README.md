## こんにちは、en6113です 👋

- 🌱 フルスタックWebアプリ開発を学習中です。
- Web系エンジニア（バックエンドエンジニア）を志望しています。

## 🛠️ 使える技術

- **バックエンド**: PHP / Laravel
- **フロントエンド**: HTML5 / CSS / Blade / JavaScript
- **データベース**: MySQL
- **その他**: Docker / Git・GitHub

## 📂 学習成果物

スクールの課題として作成したリポジトリです。
仕様に関する指示があり、提供されたビューを使用しています。
- [bookshelf-app](https://github.com/en6113/bookshelf-app) — 書籍レビューアプリ（Laravel + Sanctum + REST API + 通知機能 + 外部API連携）
- [contact-form](https://github.com/en6113/contact-form) — 問い合わせ管理ツール（Laravel + REST API）
- [task-manager](https://github.com/en6113/task-manager) — タスク管理アプリ（Laravel + REST API）
- [certify-lms](https://github.com/en6113/certify-lms) _ 資格学習プラットフォーム（Laravel + REST API + 通知機能 + 外部API連携）←実装中（チケットベース開発）


学びを深めるために自分で作成したリポジトリです。
- [stock-manager](https://github.com/en6113/stock-manager) — 在庫管理アプリ（Laravel）
- [school-kit](https://github.com/en6113/school-kit-app) — 買い物アプリ（Laravel）
- [staff-board](https://github.com/en6113/staff-board) — 職員用情報共有アプリ（Laravel）

## 🌱 これから挑戦したいこと

- 本番環境を構築する経験をしたいため、AWSを学習中です。
- Web系エンジニア（バックエンドエンジニア）として実務経験を積みたいです。
